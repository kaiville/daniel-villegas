...
<select id="materie" multiple="multiple" size="6">
...
Tieni premuto <strong>CTRL</strong> per selezionare più materie.
</p>
</div>
...
function stampaDati() {
    ...
    const selezioneSesso = document.querySelector('input[name="sesso"]:checked');
    ...
    const mezziSelezionati = document.querySelectorAll('input[name="mezzi"]:checked');
    const mezziArray = [];
    for (const i = 0; i < mezziSelezionati.length; i++) {
        mezziArray.push(mezziSelezionati[i].value);
    }
    if (mezziArray.length === 0) mezziArray = ["Nessuno"];
    user.mezzi = mezziArray;

    // Materie preferite (max 3)
    const materieSel = [];
    const options = document.getElementById("materie").selectedOptions;

    for (const k = 0; k < options.length; k++) {
        materieSel.push(options[k].value);
    }
    ...
    outputDiv.innerHTML =
        ...
        "<div><strong>Materie preferite:</strong> " + user.materie.join(", ") + "</div>";
}
</script>
